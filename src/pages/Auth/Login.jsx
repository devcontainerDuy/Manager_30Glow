...
const Login = () => {
    ...
    const loginUser = async () => {
        ...
        try {
            ...
            const { check, token, role, uid, expiry } = response;
            ...
            if (error.response) {
                error.response.status === 429 
                    ...
                    : notyf.error('Lỗi từ server: ' + error.response.data.message || 'Không xác định');
            } else {
                notyf.error(error.request ? 'Không nhận được phản hồi từ server.' : `Đã xảy ra lỗi: ${error.message}`);
            }
        } finally {
            setLoading(false);
        }
        ...
        <form className="form-signin" onSubmit={handleSubmit}>
            <div className="text-center mb-4">
                <img
                    ...
                    height={72}
                />
                <h2 className="h3 mb-3 font-weight-normal" style={{ color: "blue" }}>HỆ THỐNG QUẢN LÝ</h2>
